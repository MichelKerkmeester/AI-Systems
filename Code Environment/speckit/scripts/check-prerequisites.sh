...
# Check retrospective document
[[ -f "$RETROSPECTIVE" ]] && docs+=("retrospective.md")
...
# Check spikes directory (only if it exists and has files)
if [[ -d "$SPIKES_DIR" ]] && [[ -n "$(ls -A "$SPIKES_DIR" 2>/dev/null)" ]]; then
    docs+=("spikes/")
fi
...
if $JSON_MODE; then
    ...
    check_file "$RETROSPECTIVE" "retrospective.md"
    check_dir "$CHECKLISTS_DIR" "checklists/"
    check_dir "$DECISIONS_DIR" "decisions/"
    check_dir "$SPIKES_DIR" "spikes/"
